...
def gen_dates(time_is, time_os, date_from="2013-01-01", date_to="2017-12-31"):
    ...
    while True:
        date_from_os = date_from_is + dateutil.relativedelta.relativedelta(
            months=time_is
        )
        ...
        if date_from_os > date_to:
            ...
            list_of_dates.append(
                [
                    date_from_is.strftime("%Y-%m-%d"),
                    date_to_is.strftime("%Y-%m-%d"),
                    date_from_os.strftime("%Y-%m-%d"),
                    date_to_os.strftime("%Y-%m-%d"),
                ]
            )
            break
        ...
        date_from_is = date_from_is + dateutil.relativedelta.relativedelta(
            months=time_os
        )
