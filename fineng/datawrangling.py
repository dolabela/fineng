...
from quantfin.utils import printProgressBar, log
...
def merge_csv_to_df():
    ...
    for n, company in enumerate(list_of_csvs):
        ...
        if (n == 0):
            prices = pd.DataFrame(index= pd.read_csv(os.path.join('dataset', "stocks",company))['Unnamed: 0'])
        ...
        # Se contém preços, salva no arquivo prices a coluna com o nome da ação.
            ...
            company_data.drop(['open', 'high', 'low', 'volume_dollar','volume_shares','volume_ticks','avg'], 1, inplace=True)
...
def get_tyde():
    ...
    benchmark_tyde = pd.read_csv(os.path.join('dataset', "tyde", "portReturns.csv"), 
                                names = ['Date', 'Return'])
