...
def plot_strategy(strategy_list):
    ...
    # Apresenta o título
    fig.update_layout(title_text='Portfolios Comparison',
...
def compare_strategies(list_of_strategies,benchmark, days_back = 0, display_table = False):
    ...
    for s in list_of_strategies:
        ...
        aux = pd.DataFrame.from_dict({"Strategy": stats.name, 
...
def plot_rolling_vol(strategy_list):
    ...
    for s in strategy_list:
        ...
        fig.add_trace(go.Scatter(x=rolling_sharpe.index, 
                                y=rolling_sharpe.values, 
                                name= s.name
                                )) 

    # Apresenta o título
    fig.update_layout(title_text='Rolling Volatility 252 days',
                    xaxis_rangeslider_visible=True)
...
def plot_rolling_ret(strategy_list):
    ...
    # Apresenta o título
    fig.update_layout(title_text='Rolling Return 252 days',
                    xaxis_rangeslider_visible=True)
...
def plot_rolling_cvar(strategy_list):
    ...
    # Apresenta o título
    fig.update_layout(title_text='Rolling CVar 5% 252 days',
                    xaxis_rangeslider_visible=True)
...
def plot_rolling_sharpe(strategy_list):
    ...
    # Apresenta o título
    fig.update_layout(title_text='Rolling Sharpe 252 days',
                    xaxis_rangeslider_visible=True)
...
def plot_daily_drawdown(strategy):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=max_daily_draw_down, name='Maximum daily drawdown in time-window'))    
    fig.update_layout(title_text='Daily drawdown',
                    xaxis_rangeslider_visible=True)
    fig.show()
