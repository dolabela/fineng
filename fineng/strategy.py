...
from quantfin.utils import printProgressBar, gen_dates, log 
import quantfin.calculation as calc
import quantfin.port_optimization as p_opt


class Strategy():
    def __init__(self, method, name, rets, log_path = 'log.csv', args = False, time_is = 12, time_os = 3,min_w = 0, max_w = 0.05, date_from = '2013-01-01', date_to = '2017-12-31', threads = 1, os_equal_is = False, rsi = False, ewma=1, minimization_tolerance = None):
        ...
        if os_equal_is:
            ...
            self.list_of_dates = gen_dates(time_is = time_is, 
                                       time_os = time_os, 
                                       date_from = date_from, 
                                       date_to = date_to)
        ...
        log("start","Calculating weights from {} periods".format(len(self.list_of_dates)),3, log_path)
        p=Pool(threads)
        ...
        log("finish","Calculating weights from {} periods".format(len(self.list_of_dates)),3, log_path)  

        log("start","Simulating Strategy",5, log_path)
        self.strategy_cumret = self.calculate_return(self.rets, 
    ...
    def calculate_weights(self, dates):
        ...
        log("start", "Calculating portfolio weights from {} to {}".format(datetime.datetime.strptime(date_from_is , "%Y-%m-%d").date() ,datetime.datetime.strptime(date_to_is , "%Y-%m-%d").date() ),4,  self.log_path)
        ...
        if (self.__ewma > 1):
            ...
            rets_is = rets_is.ewm(span=self.__ewma,min_periods=0,adjust=False,ignore_na=False).mean()
        ...
        if self.method == "random" or self.method == "equally_weighted":
            portfolio = p_opt.gen_port( rets = rets_is,
            ...
            portfolio = p_opt.gen_port_optimized(rets_is , 
                                        date_from = date_from_is, 
                                        date_to = date_to_is,
                                        opt_method = self.method, 
        ...
        log("finish", "Calculating portfolio weights from {} to {}".format(datetime.datetime.strptime(date_from_is , "%Y-%m-%d").date() ,datetime.datetime.strptime(date_to_is , "%Y-%m-%d").date() ),4, self.log_path) 
        return portfolio
    ...
    def calculate_return (self, rets, weights):
        ...
        rets = deepcopy(rets)
        rets = rets.loc[weights.index[0].split('/')[0]: weights.index[-1].split('/')[1]]
        rets = rets.fillna(0)
    ...
    def print_weights(self):
        ...
        for i,row in enumerate(self.strategy_weights.iterrows()):
            ...
            if (i>=1):
                ...
                print("Stocks that left:", ((s_weights.iloc[i,:] < 0.000001)&(s_weights.iloc[i - 1,:] > 0.000001)).sum())            
                print("New Stocks:", ((s_weights.iloc[i,:] > 0.000001)&(s_weights.iloc[i - 1,:] < 0.000001)).sum())            
